...
class FavorableCardToKafkaHandle(MBHandler):
    """
    支付优惠卡调用，向kafka推送数据
    """
    @coroutine
    @use_args_query(FavorableCardToKafkaSerializer)
    def post(self, args):
        """
        tags: [优惠卡]
        summary: 支付优惠卡调用，向kafka推送数据
        description: 支付优惠卡调用，向kafka推送数据
        ...
        response = yield mb_async(FavorableCardUserService().favorable_card_to_kafka)(*valid_data)
